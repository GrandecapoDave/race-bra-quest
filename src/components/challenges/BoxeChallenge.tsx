...
export function BoxeChallenge({ challenge, team, completed, onComplete, completing }: Props) {
  ...
  const { data: matches, isLoading: loadingMatches, refetch: refetchMatches } = useQuery({
    queryKey: ["boxe_tournament", challenge.id],
    queryFn: async () => {
      const { data, error } = await supabase.rpc("get_boxe_tournament");
      if (error) throw error;
      return data || [];
    }
  });

  // 2b. Fetch tournament settings
  const { data: boxeSettings, isLoading: loadingSettings, refetch: refetchSettings } = useQuery({
    queryKey: ["boxe_settings", challenge.id],
    queryFn: async () => {
      const { data, error } = await supabase.rpc("get_boxe_settings");
      if (error) throw error;
      return data;
    }
  });
  ...
  const setSpecialByeMutation = useMutation({
    mutationFn: async (teamId: string | null) => {
      ...
      const { data, error } = await supabase.rpc("set_boxe_special_bye", {
        p_team_id: teamId,
        p_admin_id: adminId
      });
      if (error) throw error;
  ...
  useEffect(() => {
    if (boxeSettings?.special_bye_team_id) {
      setSelectedSpecialByeTeamId(boxeSettings.special_bye_team_id);
    } else if (boxeSettings) {
  ...
  const submitWinnerMutation = useMutation({
    mutationFn: async ({ matchId, winnerId }: { matchId: string; winnerId: string }) => {
      const adminId = isAdmin ? "11111111-1111-1111-1111-111111111111" : currentUserId;
      const { data, error } = await supabase.rpc("submit_boxe_match_result", {
        p_match_id: matchId,
    ...
    onSuccess: () => {
      toast.success("Risultato registrato!");
      queryClient.invalidateQueries({ queryKey: ["boxe_tournament"] });
      queryClient.invalidateQueries({ queryKey: ["my-team-score"] });
      refetchMatches();
      refetchSettings();
      onComplete(); // refresh completion status
    },
    onError: (err: any) => {
      toast.error(err.message || "Errore durante il salvataggio.");
    },
  ...
  const rollbackMutation = useMutation({
    mutationFn: async (matchId: string) => {
      const adminId = isAdmin ? "11111111-1111-1111-1111-111111111111" : currentUserId;
      const { data, error } = await supabase.rpc("rollback_boxe_match_result", {
        p_match_id: matchId,
        p_admin_id: adminId
      });
      if (error) throw error;
    ...
    onSuccess: () => {
      toast.success("Match annullato con successo!");
      queryClient.invalidateQueries({ queryKey: ["boxe_tournament"] });
      queryClient.invalidateQueries({ queryKey: ["my-team-score"] });
      refetchMatches();
      refetchSettings();
  ...
  if (loadingTeams || loadingMatches || loadingSettings) {
    return (
      <div className="flex flex-col items-center justify-center py-12 gap-3">
        ...
        <p className="text-sm text-muted-foreground">Caricamento torneo di Boxe Gonfiabile...</p>
      </div>
    );
  }
  ...
  // Determine the last completed match (for rollback)
  const completedMatches = sortedMatches.filter((m: any) => m.status === "completed" && m.team2_id !== null);
  ...
  return (
    <div className="space-y-6 w-full max-w-4xl mx-auto p-1">
      ...
      <div className="surface p-5 rounded-2xl border border-border/40 text-center space-y-2 relative overflow-hidden bg-gradient-to-b from-[#1c0f16] to-[#0d070b]">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground max-w-md mx-auto">
            Torneo fisico ad eliminazione diretta 1vs1, round secco.
            Gli accoppiamenti iniziali sono sorteggiati casualmente dal sistema.
          </p>
        </div>
      </div>

      {/* If tournament matches are not generated yet */}
      {sortedMatches.length === 0 ? (
            ...
            <div className="surface border border-primary/20 bg-primary/5 rounded-3xl p-6 space-y-6">
              <div className="border-b border-border/10 pb-3">
                <h3 className="text-sm font-black uppercase tracking-wider text-primary flex items-center gap-2">
                  ⚙️ Configurazione Torneo Boxe
                </h3>
                <p className="text-xs text-muted-foreground mt-1">
                  Configura il vantaggio speciale e poi genera il sorteggio per inizializzare il tabellone.
                </p>
              </div>
              ...
              <div className="bg-zinc-950/40 border border-[#f59e0b]/20 p-5 rounded-2xl space-y-4">
                <div>
                  <h4 className="text-xs font-black uppercase text-[#f59e0b] tracking-wider flex items-center gap-2">
                    ⚡ Vantaggio Tappa 4.3 (Boxe)
                  </h4>
                  <p className="text-[11px] text-muted-foreground mt-1">
                    La squadra arrivata prima nella Sfida 4.3 può saltare il primo turno del torneo di Boxe.
                  </p>
                </div>

                {boxeSettings?.first_place_stage4_3 && (
                  <div className="bg-[#f59e0b]/5 border border-[#f59e0b]/20 px-3 py-2 rounded-xl text-center">
                    <p className="text-[10px] text-muted-foreground font-semibold">
                      ...
                      <span className="text-[#f59e0b] font-black">{getTeamName(boxeSettings.first_place_stage4_3)}</span>
                    </p>
                  </div>
                )}
        ...
        /* If tournament matches are generated */
        <div className="space-y-6">
          {/* STATO TORNEO - Info panel */}
          <div className="surface p-5 rounded-3xl border border-border/40 bg-zinc-950/20 grid grid-cols-2 md:grid-cols-5 gap-4 text-center">
            <div>
              <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest">Squadre</p>
              <p className="text-base font-black text-foreground mt-0.5">{allTeams?.filter((t: any) => t.active !== false).length}</p>
            </div>
            <div>
              <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest">Match</p>
              <p className="text-base font-black text-foreground mt-0.5">
                {sortedMatches.filter((m: any) => m.status === "completed" && m.team2_id !== null).length} / {sortedMatches.filter((m: any) => m.team2_id !== null).length}
              </p>
            </div>
            <div>
              <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest">Turno Corrente</p>
              <p className="text-base font-black text-primary mt-0.5 uppercase">
                {isTournamentCompleted ? "Completato" : activeMatches.length > 0 ? getRoundLabel(activeMatches[0].round) : "In corso"}
              </p>
            </div>
            <div>
              <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest">Bye Speciale</p>
              <p className="text-base font-black text-[#f59e0b] mt-0.5 truncate max-w-[130px] mx-auto">
                {boxeSettings?.special_bye_team_id ? getTeamName(boxeSettings.special_bye_team_id) : "Nessuno"}
              </p>
            </div>
            <div className="col-span-2 md:col-span-1 border-t md:border-t-0 md:border-l border-border/10 pt-2 md:pt-0">
              <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest">Campione</p>
              <p className="text-base font-black text-gold mt-0.5 truncate max-w-[130px] mx-auto">
                {tournamentWinnerId ? getTeamName(tournamentWinnerId) : "—"}
              </p>
            </div>
          </div>
          ...
          {isAdmin && (
            <div className="surface border border-primary/20 bg-primary/5 rounded-3xl p-6 space-y-6">
              ...
              {isTournamentCompleted ? (
                ...
                <div className="space-y-6">
                  {/* Highlighted ACTIVE MATCH card */}
                  <div className="bg-red-950/20 border border-red-500/30 p-6 rounded-2xl text-center space-y-4 shadow-lg shadow-red-950/10">
                    <p className="text-[10px] font-black uppercase tracking-widest text-red-500 animate-pulse">
                      🔴 MATCH ATTIVO — {getRoundLabel(activeMatches[0].round)}
                    </p>
                    ...
                    <div className="flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-8">
                      ...
                      <div className="flex-1 w-full">
                        <button
                          onClick={async () => {
                            setSubmittingMatchId(activeMatches[0].id);
                            await submitWinnerMutation.mutateAsync({ matchId: activeMatches[0].id, winnerId: activeMatches[0].team1_id });
                          }}
                          disabled={submitWinnerMutation.isPending}
                          className="w-full py-4.5 px-5 bg-gradient-to-r from-blue-900/60 to-blue-700/30 hover:from-blue-800/80 border border-blue-500/40 rounded-2xl text-foreground font-black text-sm active:scale-95 transition-all cursor-pointer shadow-lg shadow-blue-950/20"
                        >
                          🏆 {getTeamName(activeMatches[0].team1_id)} VINCE
                        </button>
                      </div>

                      <span className="text-xs font-black uppercase tracking-widest text-red-500 px-3 py-1 bg-red-950/30 rounded-full border border-red-500/20 shrink-0">
                        VS
                      </span>
                      ...
                      <div className="flex-1 w-full">
                        <button
                          onClick={async () => {
                            setSubmittingMatchId(activeMatches[0].id);
                            await submitWinnerMutation.mutateAsync({ matchId: activeMatches[0].id, winnerId: activeMatches[0].team2_id });
                          }}
                          disabled={submitWinnerMutation.isPending}
                          className="w-full py-4.5 px-5 bg-gradient-to-r from-orange-900/60 to-orange-700/30 hover:from-orange-800/80 border border-orange-500/40 rounded-2xl text-foreground font-black text-sm active:scale-95 transition-all cursor-pointer shadow-lg shadow-orange-950/20"
                        >
                          🏆 {getTeamName(activeMatches[0].team2_id)} VINCE
                        </button>
                      </div>
                    </div>
                    {submitWinnerMutation.isPending && submittingMatchId === activeMatches[0].id && (
                      <div className="flex items-center justify-center gap-1.5 text-xs text-primary font-bold">
                        <Loader2 className="size-3.5 animate-spin" /> Salvataggio verdetto in corso...
                      </div>
                    )}
                  </div>

                  {/* Other pending matches list if any */}
                  {activeMatches.length > 1 && (
                    <div className="space-y-2 border-t border-border/10 pt-4">
                      <p className="text-[10px] font-black uppercase tracking-wider text-muted-foreground mb-2">Prossimi incontri in coda:</p>
                      <div className="grid gap-2">
                        {activeMatches.slice(1).map((m: any) => (
                          <div key={m.id} className="bg-zinc-900/40 border border-border/20 px-4 py-2.5 rounded-xl flex items-center justify-between text-xs">
                            <span className="font-bold text-muted-foreground">{getRoundLabel(m.round)} — Match {m.match_index + 1}</span>
                            <span className="font-black text-foreground">{getTeamName(m.team1_id)} vs {getTeamName(m.team2_id)}</span>
                          </div>
                        ))}
                      </div>
                    </div>
          ...
          {!isAdmin && !isTournamentCompleted && (
            <div className="surface p-5 rounded-2xl border border-border/40 text-center space-y-4">
              {boxeSettings?.special_bye_team_id === currentUserId ? (
                <div className="space-y-1 animate-in fade-in duration-300">
                  ...
                  <p className="text-xs text-muted-foreground">
                    <b>Salti il primo turno del torneo di Boxe</b> e accederai direttamente al turno successivo.
                  </p>
                </div>
              ) : (
      ...
      {/* 📊 PUNTEGGI SFIDA 5.2 - Visualizzazione centralizzata per Admin e Team */}
      {sortedMatches.length > 0 && (
        <div className="surface border border-border/30 rounded-3xl p-6 space-y-4">
          <div className="border-b border-border/10 pb-3 flex items-center justify-between">
            ...
            <span className={`text-[10px] font-black uppercase px-2.5 py-0.5 rounded-full ${
              isTournamentCompleted 
                ...
                : "bg-[#f59e0b]/20 border border-[#f59e0b]/30 text-[#f59e0b]"
            }`}>
              {isTournamentCompleted ? "✅ Punteggi Assegnati" : "⏳ Punteggi non ancora assegnati"}
            </span>
          </div>
          ...
          <div className="overflow-hidden rounded-xl border border-border/30 bg-zinc-950/40">
            <table className="w-full text-xs text-left">
              <thead className="bg-muted/10 text-muted-foreground uppercase text-[9px] tracking-wider border-b border-border/30">
                <tr>
                  <th className="px-4 py-2.5">Squadra</th>
                  <th className="px-4 py-2.5">Risultato</th>
                  <th className="px-4 py-2.5 text-right">Punti</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-border/25">
                {allTeams
                  ?.filter((t: any) => t.active !== false)
                  .map((t: any) => {
                    const isCampione = isTournamentCompleted && t.id === tournamentWinnerId;
                    const punti = isTournamentCompleted ? (isCampione ? 20 : 10) : 0;
                    return (
                      <tr key={t.id} className={`${isCampione ? "bg-gold/10 font-bold text-gold" : "text-muted-foreground"}`}>
                        <td className="px-4 py-2 flex items-center gap-1.5">
                          {isCampione ? "🏆" : "👥"} {t.nome_squadra}
                          {boxeSettings?.special_bye_team_id === t.id && (
                            <span className="text-[8px] bg-[#f59e0b]/20 border border-[#f59e0b]/30 text-[#f59e0b] font-black px-1.5 rounded-full uppercase">Bye</span>
                          )}
                        </td>
                        <td className="px-4 py-2">
                          {isTournamentCompleted ? (isCampione ? "Campione" : "Eliminata") : "In gara"}
                        </td>
                        <td className="px-4 py-2 text-right font-black text-foreground">
                          {isTournamentCompleted ? punti : "—"}
                        </td>
                      </tr>
                    );
                  })}
      ...
      <div className="space-y-3">
        <h3 className="text-xs font-bold uppercase tracking-widest text-muted-foreground px-1 flex items-center gap-1.5">
          Tabellone Torneo (Bracket) <span className="text-[10px] text-primary/70 animate-pulse font-semibold">(In tempo reale)</span>
        </h3>

        <div className="surface rounded-3xl border border-border/30 p-6 overflow-x-auto flex gap-8 items-start bg-[#0d070b]/80 shadow-lg min-h-[350px]">
