...
export function AppShell({
  ...
  /** Pagina che su schermo largo usa piu' spazio (es. Dashboard squadra a due colonne). */
  wide?: boolean | undefined;
}) {
  return (
    <SidebarProvider>
      <AppShellInner isAdmin={isAdmin} wide={wide}>{children}</AppShellInner>
    </SidebarProvider>
  );
}
    ...
    <div className="flex min-h-screen w-full">
      ...
      <SidebarInset className="min-w-0 w-full max-w-full flex-1 pb-24 md:pb-0 overflow-x-hidden box-border">
        <header className="sticky top-0 z-30 w-full border-b border-white/[0.06] bg-background/80 backdrop-blur-md safe-top">
          <div className={`mx-auto flex w-full max-w-3xl${isAdmin ? " md:max-w-5xl 2xl:max-w-7xl" : wide ? " lg:max-w-5xl" : ""} items-center justify-between gap-2 px-3 sm:px-4 py-3 box-border`}>
        ...
        {!isAdmin && (gameSettings.data as any)?.race_paused === true && (
          ...
          >
            <div className={`mx-auto flex w-full max-w-3xl${isAdmin ? " md:max-w-5xl 2xl:max-w-7xl" : wide ? " lg:max-w-5xl" : ""} items-center gap-2 px-3 sm:px-4 py-2 text-cyan-100`}>
        ...
        <main className={`mx-auto w-full max-w-3xl${isAdmin ? " md:max-w-5xl 2xl:max-w-7xl" : wide ? " lg:max-w-5xl" : ""} min-w-0 px-3 sm:px-4 py-4 sm:py-5 pb-28 md:pb-12 box-border overflow-x-hidden`}>
