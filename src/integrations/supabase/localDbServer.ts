...
function getDb() {
  ...
  if (!db.marketplace_items || db.marketplace_items.length === 0) {
    db.marketplace_items = [
      // BONUS
      { id: "bonus_punti", nome: "BONUS PUNTI", categoria: "BONUS", descrizione: "Un grande vantaggio per scalare la classifica. Usa questo bonus nel momento decisivo.", costo_token: 40, active: true },
      { id: "bonus_scudo", nome: "BONUS SCUDO", categoria: "BONUS", descrizione: "Uno scudo invisibile protegge il vostro viaggio dagli attacchi degli avversari.", costo_token: 35, active: true },
      { id: "ruota_fortuna", nome: "RUOTA DELLA FORTUNA", categoria: "BONUS", descrizione: "La fortuna decide il vostro destino. Siete pronti a rischiare?", costo_token: 25, active: true },
      { id: "passaparola", nome: "PASSAPAROLA", categoria: "BONUS", descrizione: "Permette di chiamare l'organizzatore una volta per ricevere un indizio extra SÌ/NO su qualsiasi enigma bloccato.", costo_token: 20, active: true },
      { id: "bonus_classifica", nome: "BONUS CLASSIFICA", categoria: "BONUS", descrizione: "Visualizza temporaneamente la classifica generale.", costo_token: 30, active: true },
      { id: "partenza_anticipata", nome: "PARTENZA ANTICIPATA", categoria: "BONUS", descrizione: "-2 minuti sulla partenza. Comunica alla Regia per utilizzarlo.", costo_token: 35, active: true },
      // MALUS
      { id: "freeze_2min", nome: "FREEZE 2 MINUTI", categoria: "MALUS", descrizione: "Il tempo si ferma per i vostri rivali.", costo_token: 20, active: true },
      { id: "enigma_extra", nome: "ENIGMA EXTRA", categoria: "MALUS", descrizione: "La squadra bersaglio deve completare un enigma aggiuntivo prima di continuare.", costo_token: 25, active: true },
      { id: "ruota_sfortunata", nome: "RUOTA SFORTUNATA", categoria: "MALUS", descrizione: "La squadra bersaglio deve girare una ruota con possibili penalità casuali.", costo_token: 20, active: true },
      { id: "trappola", nome: "TRAPPOLA", categoria: "MALUS", descrizione: "Ruba fino a 30 Punti Squadra a una squadra avversaria.", costo_token: 40, active: true },
      { id: "penalita_punti", nome: "PENALITÀ PUNTI (-20 PT)", categoria: "MALUS", descrizione: "−20 PT", costo_token: 30, active: true },
      { id: "tassa_passaggio", nome: "TASSA DI PASSAGGIO", categoria: "MALUS", descrizione: "Scambia integralmente i Punti Squadra correnti con quelli di una squadra avversaria.", costo_token: 70, active: true }
    ];
    updated = true;
  }
  ...
  if (!db.game_settings || db.game_settings.length === 0) {
    db.game_settings = [
      { id: "settings_01", marketplace_visible: false, marketplace_active: false, activated_at: null, activated_by: null, cornhole_special_bye_team_id: null, boxe_special_bye_team_id: null }
    ];
    updated = true;
  } else {
  ...
  if (stage1 && (stage1.descrizione !== "Piazza Caduti per la Libertà, 14" || stage1.latitude !== 44.6982 || stage1.longitude !== 7.8507)) {
  ...
  if (stage2 && (stage2.nome_tappa !== "Il Rebus Visivo" || stage2.descrizione !== "Via Mendicità Istruita, 12" || stage2.latitude !== 44.6976 || stage2.longitude !== 7.8544)) {
  ...
  if (!stage3Exists) {
    ...
    if (stage3 && (stage3.nome_tappa !== "La Banca" || stage3.descrizione !== "Stazione Ferroviaria di Bra" || stage3.latitude !== 44.6946 || stage3.longitude !== 7.8542)) {
  ...
  const rebusVisivo = db.challenges.find((c: any) => c.id === "999f4e1f-7443-42e7-9d7a-115f2122888f");
  if (!rebusVisivo) {
    db.challenges.push({
    ...
    updated = true;
  } else if (rebusVisivo.tipo_sfida !== "photo" || rebusVisivo.punteggio_massimo !== 25 || rebusVisivo.titolo !== "Il Rebus Visivo") {
  ...
  const emojiMoviesCh = db.challenges.find((c: any) => c.id === "777f4e1f-7443-42e7-9d7a-115f2122888f");
  if (!emojiMoviesCh) {
    db.challenges.push({
      id: "777f4e1f-7443-42e7-9d7a-115f2122888f",
      stage_id: "dfa9e6db-4e1b-41be-94be-21cf2980fa2a",
      titolo: "Indovina il film dalle emoji",
      descrizione: "Viaggiatori, si spengono le luci, si alza il sipario: benvenuti nella sala più insolita della caccia!",
      tipo_sfida: "emoji_movies",
      punteggio_massimo: 15,
      ordine: 2
    });
    updated = true;
  } else if (emojiMoviesCh.ordine !== 2 || emojiMoviesCh.tipo_sfida !== "emoji_movies" || emojiMoviesCh.punteggio_massimo !== 15) {
  ...
  const livingPosterCh = db.challenges.find((c: any) => c.id === "555f4e1f-7443-42e7-9d7a-115f2122888f");
  if (!livingPosterCh) {
    db.challenges.push({
    ...
    updated = true;
  } else if (livingPosterCh.ordine !== 3 || livingPosterCh.tipo_sfida !== "living_poster" || livingPosterCh.punteggio_massimo !== 15) {
  ...
  const bankAnswersCount = db.challenge_answers.filter((a: any) => a.challenge_id === "b1b2b3b4-b5b6-b7b8-b9b0-b1b2b3b4b5b6").length;
  if (bankAnswersCount === 0) {
    db.challenge_answers.push(
      { id: "ans_1", challenge_id: "b1b2b3b4-b5b6-b7b8-b9b0-b1b2b3b4b5b6", question_number: 1, question_text: "Lo usi per prelevare contanti senza fare la fila allo sportello", correct_answer: "BANCOMAT", extracted_letter: "B" },
      { id: "ans_2", challenge_id: "b1b2b3b4-b5b6-b7b8-b9b0-b1b2b3b4b5b6", question_number: 2, question_text: "Il codice segreto a 4 cifre che non devi mai dire a nessuno", correct_answer: "PIN", extracted_letter: "P" },
      { id: "ans_3", challenge_id: "b1b2b3b4-b5b6-b7b8-b9b0-b1b2b3b4b5b6", question_number: 3, question_text: "La moneta che hai in tasca in tutta Europa", correct_answer: "EURO", extracted_letter: "E" },
      { id: "ans_4", challenge_id: "b1b2b3b4-b5b6-b7b8-b9b0-b1b2b3b4b5b6", question_number: 4, question_text: "La scadenza mensile del mutuo, incubo di ogni famiglia", correct_answer: "RATA", extracted_letter: "R" }
    );
    updated = true;
  }
  ...
  if (!bankCh) {
    db.challenges.push({
      ...
      titolo: "La Banca",
      descrizione: "Quattro indizi, quattro parole, Viaggiatori. Risolveteli come veri enigmisti da settimana enigmistica: una definizione, una risposta, una sola letter che conta davvero — la prima.",
      tipo_sfida: "banca",
      punteggio_massimo: 25,
      ordine: 1,
    ...
    updated = true;
  } else if (bankCh.ordine !== 1 || bankCh.tipo_sfida !== "banca" || bankCh.unlock_condition !== "marketplace_active" || bankCh.punteggio_massimo !== 25) {
    bankCh.ordine = 1;
  ...
  if (!socialCh) {
    db.challenges.push({
      ...
      descrizione: "Viaggiatori, questa volta la sfida non è contro il tempo, ma contro la vostra capacità di entrare in contatto con il mondo. Dimostrate di saper comunicare, convincere e creare un legame con persone mai incontrate prima.",
      tipo_sfida: "social",
    ...
    updated = true;
  } else if (socialCh.ordine !== 2 || socialCh.tipo_sfida !== "social" || socialCh.unlock_condition !== "marketplace_active" || socialCh.punteggio_massimo !== 20) {
    socialCh.ordine = 2;
  ...
  if (!codeCh) {
    db.challenges.push({
      ...
      descrizione: "Viaggiatori, per sbloccare la destinazione finale della gara dovete inserire il PIN a 10 cifre. Ma ricordate: voi avete solo mezza chiave. Dovete trovare il vostro partner economico e acquistare il frammento mancante usando i vostri Token.",
      tipo_sfida: "codice",
      punteggio_massimo: 15,
      ordine: 3,
      unlock_condition: "marketplace_active"
    ...
    updated = true;
  } else if (codeCh.ordine !== 3 || codeCh.tipo_sfida !== "codice" || codeCh.unlock_condition !== "marketplace_active" || codeCh.punteggio_massimo !== 15) {
    codeCh.ordine = 3;
  ...
  const hasStazione = db.challenges.some((c: any) => c.id === "888f4e1f-7443-42e7-9d7a-115f2122888f");
  if (hasStazione) {
    db.challenges = db.challenges.filter((c: any) => c.id !== "888f4e1f-7443-42e7-9d7a-115f2122888f");
    updated = true;
  }
  ...
  if (!stage4Exists) {
    ...
    if (s4 && s4.ordine !== 4) { s4.ordine = 4; updated = true; }
  }
  ...
  if (!enigma1Exists) {
    db.challenges.push({
      ...
      descrizione: "Ricevete il rebus cartaceo, scoprite le 3 note e inseritele nell'ordine corretto.",
  ...
  if (!jackpotExists) {
    db.challenges.push({
      ...
      descrizione: "Sfida Bonus opzionale. Sfida la fortuna alla slot machine scommettendo i tuoi punti.",
      tipo_sfida: "jackpot",
      punteggio_massimo: 20,
      ordine: 3
    });
  ...
  if (db.challenges.some((c: any) => c.id === TRAGUARDO_CHALLENGE_ID)) {
    ...
    db.team_progress = db.team_progress?.filter((tp: any) => tp.challenge_id !== TRAGUARDO_CHALLENGE_ID) ?? [];
    updated = true;
  }
...
function calculateLeaderboard(db: any) {
  ...
  const rows = db.teams.map((t: any) => {
    ...
    const cattiveriaPoints = (db.cattiveria_ledger || []).filter((l: any) => l.team_id === t.id).reduce((sum: number, l: any) => sum + l.punti, 0);
    const totalPoints = challengesPoints + modifierPoints + cattiveriaPoints;

    const progress = db.team_progress.filter((p: any) => p.team_id === t.id && p.stato === "completed" && p.challenge_id !== JACKPOT_CHALLENGE_ID);
    ...
    db.stages.forEach((s: any) => {
      ...
      const stageProgs = teamProg.filter((p: any) => stageChs.some((c: any) => c.id === p.challenge_id));
      if (stageProgs.length === 0) return;

      const startTimes = stageProgs.map((p: any) => p.started_at ? new Date(p.started_at).getTime() : 0).filter(Boolean);
      if (startTimes.length === 0) return;
      const minStart = Math.min(...startTimes);

      const completedChs = stageProgs.filter((p: any) => p.stato === "completed");
      const allCompleted = stageChs.every((c: any) => completedChs.some((p: any) => p.challenge_id === c.id));

      if (allCompleted) {
        const completionTimes = completedChs.map((p: any) => p.completata_at ? new Date(p.completata_at).getTime() : 0).filter(Boolean);
    ...
    const penaltySeconds = teamPenalties.reduce((sum: number, p: any) => sum + (p.duration || 0), 0);
    totalDurationSeconds += penaltySeconds;

    const completions = progress.map((p: any) => p.completata_at ? new Date(p.completata_at).getTime() : 0).filter(Boolean);
    ...
    return {
      ...
      motto: t.motto || "In corsa per la vittoria!",
...
function addCattiveriaPoints(
  ...
  if (txId && db.cattiveria_ledger.some((l: any) => l.riferimento_transazione === txId && l.team_id === teamId)) {
    return;
  }
...
function generateGameReport(db: any) {
  ...
  const teamsReport = sortedLeaderboard.map((teamRank: any, rankIndex: number) => {
    ...
    const stagesBreakdown = stagesList.map((stage: any) => {
      const stageChs = challengesList.filter((c: any) => c.stage_id === stage.id).sort((a: any, b: any) => a.ordine - b.ordine);
      ...
      const stageChallengesDetails = stageChs.map((c: any) => {
        ...
        const sub = submissionsList.find((s: any) => s.challenge_id === c.id && s.team_id === teamId);
        return {
          challenge_id: c.id,
          title: c.titolo,
          type: c.tipo_sfida,
      ...
      const stageChallengesPoints = stageChallengesDetails.reduce((sum: number, c: any) => sum + c.points_awarded, 0);

      // Bonuses used
      const bonusesInStage = teamTxBought.filter((tx: any) => {
      ...
      const malusesInStage = teamTxBought.filter((tx: any) => {
        const item = db.marketplace_items?.find((i: any) => i.id === tx.item_id);
      ...
      const stageCattiveriaPoints = stageCattiveriaEntries.reduce((sum: number, l: any) => sum + l.punti, 0);

      // Stage closing reward for this stage
      const stageRewardTx = teamTxBought.find((tx: any) => tx.item_id === "reward_stage" && (tx.outcome?.stage_id === stage.id || tx.outcome?.stage_index === stage.ordine));
      const endOfStageCattiveria = stageCattiveriaEntries.find((l: any) => l.tipo === "end_of_stage");
    ...
    teamTxBought.forEach((tx: any) => {
      const item = db.marketplace_items?.find((i: any) => i.id === tx.item_id);
      const targetTeam = tx.target_team_id ? db.teams.find((t: any) => t.id === tx.target_team_id) : null;
    ...
    teamCattiveria.filter((l: any) => l.tipo === "end_of_stage").forEach((l: any) => {
    ...
    try {
      ...
      if (currentUserId && (action === "rpc" || action === "mutation" || action === "upload")) {
        ...
        const activeEnigmaTx = db.marketplace_transactions?.find(
          (t: any) => t.target_team_id === currentUserId && t.item_id === "enigma_extra" && t.stato === "completed"
        );
        if (activeEnigmaTx) {
          let isBlocked = false;
          if (action === "mutation" || action === "upload") {
        ...
        const activeUnluckyWheelTx = db.marketplace_transactions?.find(
          (t: any) => t.target_team_id === currentUserId && t.item_id === "ruota_sfortunata" && t.stato === "completed"
        );
      ...
      if (action === "query") {
        ...
        if (table === "quiz_questions_public") {
          ...
          rows = rows.map((r: any) => {
            ...
            const stage = challenge ? db.stages.find((s: any) => s.id === challenge.stage_id) : null;
            return {
              ...r,
        ...
        if (select && select !== "*" && select.includes(":")) {
          const aliasFields = select.split(",").map((f: string) => f.trim()).filter((f: string) => f.includes(":"));
          rows = rows.map((r: any) => {
            const mapped = { ...r };
        ...
        if (table === "leaderboard") {
          const isAdmin = currentUserId === "justdave" || db.admin?.some((a: any) => a.id === currentUserId || a.username === currentUserId);
          const hasBonus = currentUserId && db.marketplace_transactions?.some(
        ...
        if (method === "insert") {
          ...
          const newRows = insertData.map((d: any) => {
            ...
            if (table === "teams") {
              ...
              logActivity(db, newRow.id, `Squadra "${newRow.nome_squadra}" creata ed iscritta alla competizione.`);
            }
            ...
            if (table === "submissions") {
              ...
              if (team && challenge) {
                logActivity(db, newRow.team_id, `Squadra "${team.nome_squadra}" ha inviato la prova "${challenge.titolo}" (in attesa di approvazione).`);
              }
            }
            ...
            if (matches) {
              ...
              if (table === "teams") {
                if (data.active !== undefined && data.active !== r.active) {
                  logActivity(db, r.id, `La regia ha ${data.active ? "ATTIVATO" : "DISATTIVATO"} l'account della squadra "${r.nome_squadra}".`);
                } else {
                  logActivity(db, r.id, `La regia ha modificato le informazioni per la squadra "${data.nome_squadra || r.nome_squadra}".`);
                }
              }
      ...
      if (action === "rpc") {
        ...
        if (fnName === "has_role") {
          ...
          const roleRecord = db.user_roles.find((ur: any) => ur.user_id === _user_id && ur.role === _role);
        ...
        if (fnName === "start_challenge") {
          ...
          if (challenge) {
            const exists = db.team_progress.some((tp: any) => tp.team_id === currentUserId && tp.challenge_id === p_challenge);
            if (!exists) {
              ...
              const team = db.teams.find((t: any) => t.id === currentUserId);
              if (team) {
                logActivity(db, currentUserId, `Squadra "${team.nome_squadra}" ha iniziato la prova "${challenge.titolo}".`);
              }
              saveDb(db);
            }
          }
        ...
        if (fnName === "complete_challenge") {
          const { p_challenge } = args;
          const progRecord = db.team_progress.find((tp: any) => tp.team_id === currentUserId && tp.challenge_id === p_challenge);
          ...
          const isPhotoChallenge = challenge && (
          ...
          if (!already) {
            ...
            if (challenge) {
              const allStageChs = db.challenges.filter((c: any) => c.stage_id === challenge.stage_id);
              const completedStageChs = db.team_progress.filter((tp: any) => tp.team_id === currentUserId && tp.stage_id === challenge.stage_id && tp.stato === "completed");
            ...
            if (p_challenge === "0147e750-f0a3-4b72-8e76-a003fe2ef143") {
              ...
              if (settings && !settings.marketplace_visible) {
                ...
                logActivity(db, "system", "Il Marketplace è stato SCOPERTO! La voce è ora visibile a tutti i partecipanti.");
              }
            }
          }
        ...
        if (fnName === "approve_submission") {
          ...
          if (sub) {
            ...
            db.submissions = db.submissions.map((s: any) => s.id === p_submission_id ? { ...s, stato_approvazione: "approved" } : s);
            
            // Complete team progress — upsert if not exists
            const existingProgress = db.team_progress.find((tp: any) => tp.team_id === sub.team_id && tp.challenge_id === sub.challenge_id);
            if (existingProgress) {
              db.team_progress = db.team_progress.map((tp: any) => {
        ...
        if (fnName === "reject_submission") {
          ...
          if (sub) {
            ...
            db.submissions = db.submissions.map((s: any) => s.id === p_submission_id ? { ...s, stato_approvazione: "rejected", risposta: rejectionMsg } : s);
        ...
        if (fnName === "get_or_assign_poster") {
          ...
          let assigned = db.team_posters.find((tp: any) => tp.team_id === p_team_id);
          if (assigned) {
        ...
        if (fnName === "evaluate_poster") {
          ...
          const team = db.teams.find((t: any) => t.id === sub.team_id);
          if (team && challenge) {
            logActivity(db, p_admin_id, `Regia ha assegnato ${p_voto} punti alla squadra "${team.nome_squadra}" per la sfida "${challenge.titolo}".`, p_voto);
          }
        ...
        if (fnName === "buy_marketplace_item") {
          ...
          if (!settings || !settings.marketplace_active || !settings.marketplace_visible) {
            return { data: null, error: { message: "Il Marketplace è chiuso. Non puoi effettuare acquisti." } };
          }
          ...
          if (!isTappa1Completed) {
            return { data: null, error: { message: "Il Marketplace si sbloccherà solo dopo aver completato la Tappa 1!" } };
          }
          ...
          if (item.categoria === "MALUS") {
            ...
            if (item.id === "enigma_extra") {
              const hasActiveEnigma = db.marketplace_transactions?.some(
                (t: any) => t.target_team_id === p_target_team_id && t.item_id === "enigma_extra" && t.stato === "completed"
              );
              if (hasActiveEnigma) {
                return { data: null, error: { message: "La squadra bersaglio ha già un Enigma Extra attivo!" } };
              }
            }
            ...
            if (item.id === "ruota_sfortunata") {
              const hasActiveRuota = db.marketplace_transactions?.some(
                (t: any) => t.target_team_id === p_target_team_id && t.item_id === "ruota_sfortunata" && t.stato === "completed"
              );
              if (hasActiveRuota) {
                return { data: null, error: { message: "La squadra bersaglio ha già una Ruota Sfortunata da girare!" } };
              }
            }
          }
          ...
          if (item.categoria === "MALUS" && targetTeam) {
            const activeShieldTx = db.marketplace_transactions.find(
              (t: any) => t.buyer_team_id === targetTeam.id && t.item_id === "bonus_scudo" && t.stato === "completed"
            );
            if (activeShieldTx) {
              // Consume the shield
          ...
          if (item.id === "ruota_fortuna") {
            ...
            if (rand < 3) {
              outcome = { id: "jackpot", label: "🏆 JACKPOT", points: 20, tokens: 0, daveHelp: false };
            } else if (rand < 6) {
              outcome = { id: "dave_help", label: "🧠 AIUTO EXTRA DI DAVE", points: 0, tokens: 0, daveHelp: true };
            } else if (rand < 13) {
              outcome = { id: "mega_bonus", label: "💎 MEGA BONUS", points: 15, tokens: 0, daveHelp: false };
            } else if (rand < 25) {
              outcome = { id: "bonus", label: "⭐ BONUS", points: 10, tokens: 0, daveHelp: false };
            } else if (rand < 45) {
              outcome = { id: "piccolo_bonus", label: "🎁 PICCOLO BONUS", points: 5, tokens: 0, daveHelp: false };
            } else if (rand < 55) {
              outcome = { id: "gettoni_bonus", label: "🪙 GETTONI BONUS", points: 0, tokens: 10, daveHelp: false };
            } else if (rand < 65) {
              outcome = { id: "doppio_premio", label: "🎯 DOPPIO PREMIO", points: 5, tokens: 5, daveHelp: false };
            } else if (rand < 80) {
              outcome = { id: "fortuna", label: "🍀 FORTUNA", points: 0, tokens: 5, daveHelp: false };
            } else {
              outcome = { id: "sorpresa", label: "🎉 SORPRESA", points: 3, tokens: 0, daveHelp: false };
            }
          ...
          if (item.id === "freeze_2min" && targetTeam) {
            ...
            logActivity(db, p_target_team_id, `La squadra "${targetTeam.nome_squadra}" è stata congelata da "${team.nome_squadra}" per 120 secondi!`);
          }
          ...
          if (item.id === "enigma_extra" && targetTeam) {
            ...
            logActivity(db, p_target_team_id, `La squadra "${targetTeam.nome_squadra}" ha ricevuto un Enigma Extra da "${team.nome_squadra}"!`);
          }

          if (item.id === "trappola" && targetTeam) {
            const targetScores = db.scores.filter((s: any) => s.team_id === targetTeam.id);
            const targetCurrentPoints = targetScores.reduce((sum: number, s: any) => sum + s.punti, 0);
            ...
            const buyerCurrentPoints = buyerScores.reduce((sum: number, s: any) => sum + s.punti, 0);
        ...
        if (fnName === "submit_passaparola_request") {
          ...
          if (tr.buyer_team_id !== currentUserId) return { data: null, error: { message: "Non autorizzato" } };
        ...
        if (fnName === "respond_passaparola_request") {
          ...
          const tr = db.marketplace_transactions.find((t: any) => t.id === p_transaction_id);
          if (!tr) return { data: null, error: { message: "Transazione non trovata" } };
          if (tr.stato !== "pending") return { data: null, error: { message: "La richiesta non è in attesa di risposta" } };
        ...
        if (fnName === "open_classifica_bonus") {
          ...
          const transaction = db.marketplace_transactions?.find(
            (t: any) => t.id === p_transaction_id && t.buyer_team_id === currentUserId && t.item_id === "bonus_classifica"
          );
        ...
        if (fnName === "mark_partenza_used") {
          ...
          const ADMIN_ID = "11111111-1111-1111-1111-111111111111";
          const isAdminUser = p_admin_id === ADMIN_ID || db.user_roles.some((ur: any) => ur.user_id === p_admin_id && ur.role === "admin");
          if (!p_admin_id || !isAdminUser) return { data: null, error: { message: "Non autorizzato" } };
          ...
          logActivity(db, tx.buyer_team_id, `La Regia ha registrato l'utilizzo del Bonus Partenza Anticipata da parte di "${team?.nome_squadra || "Sconosciuta"}"`);
          return { data: { success: true }, error: null };
        }
        ...
        if (fnName === "close_stage") {
          ...
          const results = teamStats.map((stat: any, index: number) => {
            ...
            const maluses = (db.cattiveria_ledger || []).filter(
              (l: any) => l.team_id === stat.team_id && l.stage_id === p_stage_id && l.tipo === "malus"
            );
        ...
        if (fnName === "reopen_stage") {
          ...
          logActivity(db, "system", `STAGE_REOPENED: La Regia ha riaperto la Tappa "${stage.nome_tappa}" (${stage.ordine}). Token e Punti Cattiveria di fine tappa sono stati revocati.`);
          saveDb(db);
          return { data: { success: true }, error: null };
        }
        ...
        if (fnName === "submit_quiz_answer") {
          ...
          db.team_answers = db.team_answers.filter((ta: any) => !(ta.team_id === currentUserId && ta.question_id === p_question));
          ...
          const hasSub = db.submissions.some((s: any) => s.team_id === currentUserId && s.challenge_id === q.challenge_id);
          if (!hasSub) {
            db.submissions.push({
              id: uuid(),
              team_id: currentUserId,
        ...
        if (fnName === "get_bank_state") {
          ...
          const progress = db.team_challenge_progress?.find(
            (p: any) => p.team_id === targetTeamId && p.challenge_id === "b1b2b3b4-b5b6-b7b8-b9b0-b1b2b3b4b5b6"
          ) || null;

          if (!db.team_challenge_answers) db.team_challenge_answers = [];
          const answers = db.team_challenge_answers.filter(
            (a: any) => a.team_id === targetTeamId && a.challenge_id === "b1b2b3b4-b5b6-b7b8-b9b0-b1b2b3b4b5b6" && a.correct
          );
        ...
        if (fnName === "submit_bank_answer") {
          ...
          if (!settings || !settings.marketplace_active) {
            return { data: null, error: { message: "La sfida è bloccata. Il Regista deve prima attivare il Marketplace." } };
          }

          const challengeId = "b1b2b3b4-b5b6-b7b8-b9b0-b1b2b3b4b5b6";
          const q = db.challenge_answers.find(
            (ca: any) => ca.challenge_id === challengeId && ca.question_number === p_question_number
          );
          ...
          const alreadyCorrect = db.team_challenge_answers.some(
            (a: any) => a.team_id === currentUserId && a.challenge_id === challengeId && a.question_number === p_question_number && a.correct
          );
          ...
          if (isCorrect && !alreadyCorrect) {
            ...
            const correctAnswers = db.team_challenge_answers.filter(
              (a: any) => a.team_id === currentUserId && a.challenge_id === challengeId && a.correct
            );
            ...
            if (correctAnswers.length === 4) {
              ...
              let generalProg = db.team_progress.find(
                (p: any) => p.team_id === currentUserId && p.challenge_id === challengeId
              );
              if (generalProg) {
            ...
            saveDb(db);
            return { data: { correct: true, letter: q.extracted_letter, challenge_completed: progress.status === "COMPLETED" }, error: null };
          }
        ...
        if (fnName === "admin_force_complete_bank") {
          ...
          if (!db.team_challenge_answers) db.team_challenge_answers = [];
          const allQuestions = db.challenge_answers.filter((q: any) => q.challenge_id === challengeId);
          allQuestions.forEach((q: any) => {
            const alreadyCorrect = db.team_challenge_answers.some(
              (a: any) => a.team_id === p_team_id && a.challenge_id === challengeId && a.question_number === q.question_number && a.correct
            );
            if (!alreadyCorrect) {
              db.team_challenge_answers.push({
                id: uuid(),
          ...
          // Submission BPER
          const hasSub = db.submissions.some((s: any) => s.team_id === p_team_id && s.challenge_id === challengeId);
          if (!hasSub) {
            db.submissions.push({
              id: uuid(),
              team_id: p_team_id,
        ...
        if (fnName === "admin_edit_bank_answer") {
          ...
          db.team_challenge_answers = db.team_challenge_answers.filter(
            (a: any) => !(a.team_id === p_team_id && a.challenge_id === challengeId && a.question_number === p_question_number)
          );
          ...
          if (p_correct) {
            ...
            if (db.scores) {
              db.scores = db.scores.filter(
                (s: any) => !(s.team_id === p_team_id && s.challenge_id === challengeId && s.motivazione.includes(`enigma ${p_question_number}`))
              );
            }
            db.scores.push({
              id: uuid(),
        ...
        if (fnName === "get_social_submission") {
          ...
          const submission = db.team_social_submissions.find(
            (s: any) => s.team_id === targetTeamId && s.challenge_id === "c2c3c4c5-c6c7-c8c9-d0d1-d2d3d4d5d6d7"
          ) || null;
        ...
        if (fnName === "submit_social_challenge") {
          ...
          if (!p_image_1_path || !p_image_2_path) {
            return { data: null, error: { message: "Devi caricare entrambe le immagini per completare la missione." } };
          }
        ...
        if (fnName === "initialize_secret_code_challenge") {
          ...
          if (activeTeams.length < 2) {
            return { data: { success: false, message: "Servono almeno 2 squadre per iniziare la sfida" }, error: null };
          }

          const finalCodeRecord = db.game_final_code?.[0];
          if (!finalCodeRecord) {
            return { data: { success: false, message: "Codice finale non inizializzato" }, error: null };
          }
          ...
          activeTeams.forEach((team: any, index: number) => {
            let part = db.team_code_parts.find((p: any) => p.team_id === team.id);
            if (!part) {
          ...
          const allMatched = activeTeams.every((t: any) => db.team_code_matches.some((m: any) => m.buyer_team_id === t.id));
        ...
        if (fnName === "get_secret_code_state") {
          ...
          const sellerTeam = match ? db.teams.find((t: any) => t.id === match.seller_team_id) : null;
          const hasPurchased = db.code_purchase_transactions?.some((t: any) => t.buyer_team_id === targetTeamId) || false;
          
          let purchasedDigits = null;
          if (hasPurchased) {
            const tx = db.code_purchase_transactions.find((t: any) => t.buyer_team_id === targetTeamId);
            purchasedDigits = tx?.digits_received || (part.part_type === "FIRST_5" ? last5 : first5);
          }

          const challengeId = "d3d4d5d6-d7d8-d9d0-e1e2-e3e4e5e6e7e8";
          const progress = db.team_progress?.find((p: any) => p.team_id === targetTeamId && p.challenge_id === challengeId);
          const completed = progress?.stato === "completed";

          return {
            data: {
              part,
              match: match ? { ...match, seller_name: sellerTeam?.nome_squadra || "Sconosciuta" } : null,
              has_purchased: hasPurchased,
              purchased_digits: purchasedDigits,
              completed,
              destination: finalCodeRecord?.next_stage_destination || "Parco Giochi Madonna dei Fiori (lato piazzale grigio)"
            },
            error: null
          };
        }
        ...
        if (fnName === "buy_secret_code_part") {
          ...
          if (!match) return { data: null, error: { message: "Nessun partner di acquisto associato" } };

          const hasPurchased = db.code_purchase_transactions?.some((t: any) => t.buyer_team_id === currentUserId);
          if (hasPurchased) return { data: null, error: { message: "Hai già acquistato il frammento" } };

          const buyerTeam = db.teams.find((t: any) => t.id === currentUserId);
          if (!buyerTeam) return { data: null, error: { message: "Squadra compratrice non trovata" } };
          ...
          const finalCodeRecord = db.game_final_code?.[0];
          const fullCode = finalCodeRecord?.full_code || "4829167305";
          const digits = match.required_part === "FIRST_5" ? fullCode.slice(0, 5) : fullCode.slice(5);
        ...
        if (fnName === "submit_secret_code_pin") {
          ...
          if (isCorrect) {
            ...
            saveDb(db);
            return { data: { success: true, destination: finalCodeRecord?.next_stage_destination || "Parco Giochi Madonna dei Fiori (lato piazzale grigio)" }, error: null };
          } else {
            logActivity(
              db,
              currentUserId,
            ...
            saveDb(db);
            return { data: { success: false, message: "Codice errato. Controllate attentamente le cifre ricevute." }, error: null };
          }
        }
        ...
        if (fnName === "admin_get_secret_code_dashboard") {
          const finalCodeRecord = db.game_final_code?.[0];
          const fullCode = finalCodeRecord?.full_code || "4829167305";
          const destination = finalCodeRecord?.next_stage_destination || "Parco Giochi Madonna dei Fiori (lato piazzale grigio)";
          ...
          const challengeId = "d3d4d5d6-d7d8-d9d0-e1e2-e3e4e5e6e7e8";
          const progress = db.team_progress?.filter((p: any) => p.challenge_id === challengeId && p.stato === "completed") || [];
        ...
        if (fnName === "admin_edit_secret_code_match") {
          const { p_buyer_team_id, p_seller_team_id, p_assigned_part_type, p_token_cost, p_admin_id } = args;
          const ADMIN_ID = "11111111-1111-1111-1111-111111111111";
          const isAdminUser = p_admin_id === ADMIN_ID || db.user_roles?.some((ur: any) => ur.user_id === p_admin_id && ur.role === "admin");
          if (!p_admin_id || !isAdminUser) {
            return { data: null, error: { message: "Non autorizzato" } };
          ...
          if (p_assigned_part_type) {
            let part = db.team_code_parts.find((p: any) => p.team_id === p_buyer_team_id);
            const codeVal = p_assigned_part_type === "FIRST_5" ? fullCode.slice(0, 5) : fullCode.slice(5);
            if (part) {
          ...
          // Update match
          let match = db.team_code_matches.find((m: any) => m.buyer_team_id === p_buyer_team_id);
        ...
        if (fnName === "admin_edit_secret_code_settings") {
          ...
          if (db.game_final_code.length === 0) {
            db.game_final_code.push({
              ...
              next_stage_destination: p_destination || "Parco Giochi Madonna dei Fiori (lato piazzale grigio)",
              created_at: new Date().toISOString()
            });
          } else {
            db.game_final_code[0].full_code = p_full_code || "4829167305";
            db.game_final_code[0].next_stage_destination = p_destination || "Parco Giochi Madonna dei Fiori (lato piazzale grigio)";
          }
        ...
        if (fnName === "submit_enigma_answer") {
          ...
          const solution = (db.enigma_solutions || []).find((s: any) => s.challenge_id === p_challenge_id);
          if (!solution) return { data: null, error: { message: "Soluzione non trovata per questo enigma" } };

          // Check if already completed
          const alreadyCompleted = (db.team_progress || []).some(
            (tp: any) => tp.team_id === currentUserId && tp.challenge_id === p_challenge_id && tp.stato === "completed"
          );
          if (alreadyCompleted) {
            return { data: { is_correct: true, already_completed: true, attempt_number: 0, points: solution.punteggio }, error: null };
          }
          ...
          if (solution.solution_type === "notes") {
            ...
            isCorrect =
              ...
              correctNotes.every((n: string, i: number) => n.toLowerCase() === submittedNotes[i]?.toLowerCase());
            ...
            isCorrect =
              ...
              correctDirections.every((d: string, i: number) => d.toLowerCase() === submittedDirections[i]?.toLowerCase());
            ...
            const norm = (val: any) => String(val || "").trim().replace(",", ".");
            isCorrect = norm(correctCoords.lat) === norm(submitted.lat) && norm(correctCoords.lng) === norm(submitted.lng);
          } else {
        ...
        if (fnName === "submit_enigma_extra_answer") {
          ...
          if (!enigmaTx) {
            return { data: null, error: { message: "Nessun Enigma Extra attivo trovato per la tua squadra." } };
          }
          ...
          if (isCorrect) {
            ...
            const team = db.teams.find((t: any) => t.id === currentUserId);
            logActivity(db, currentUserId, `La squadra "${team?.nome_squadra || "?"}" ha risolto l'Enigma Extra!`);
            saveDb(db);
          }
        ...
        if (fnName === "spin_unlucky_wheel") {
          ...
          if (!unluckyTx) {
            return { data: null, error: { message: "Nessuna Ruota Sfortunata attiva trovata per la tua squadra." } };
          }
          ...
          logActivity(db, currentUserId, `La squadra "${team.nome_squadra}" ha girato la Ruota Sfortunata ottenendo: ${selectedOutcome.label}`);
          saveDb(db);
        ...
        if (fnName === "admin_get_enigma_dashboard") {
          ...
          const rows = db.teams.map((t: any) => {
            ...
            const completedCount = enigmaProgress.filter((p: any) => p.stato === "completed").length;
        ...
        if (fnName === "admin_update_enigma_solution") {
          ...
          if (!db.enigma_solutions) return { data: null, error: { message: "Tabella soluzioni non trovata" } };
          const sol = db.enigma_solutions.find((s: any) => s.challenge_id === p_challenge_id);
          if (!sol) return { data: null, error: { message: "Soluzione non trovata per questo enigma" } };

          sol.solution = String(p_solution).trim().toUpperCase();
          logActivity(db, p_admin_id, `Regia ha aggiornato la soluzione per l'enigma: challenge_id=${p_challenge_id}`);
          saveDb(db);
          return { data: { success: true }, error: null };
        }
        ...
        if (fnName === "generate_cornhole_tournament") {
          ...
          if (specialByeTeamId && pool.some((t: any) => t.id === specialByeTeamId)) {
            ...
            const r0Matches = Math.floor(pool.length / 2);

            let tIdx = 0;
            for (let m = 0; m < r0Matches; m++) {
              matchesToInsert.push({
                ...
                round: 0,
                match_index: m,
                team1_id: pool[tIdx++].id,
                team2_id: pool[tIdx++].id,
                winner_id: null,
                status: "ready",
                completed_at: null,
            ...
            matchesToInsert.push({
              id: "match_cornhole_1_0_" + Math.random().toString(36).substr(2, 9),
              stage_id: STAGE5_ID_CONST,
              challenge_id: CORNHOLE_CHALLENGE_ID_CONST,
              round: 1,
              match_index: 0,
              team1_id: specialByeTeamId,
              team2_id: null,
              winner_id: null,
              status: "pending",
              completed_at: null,
            ...
            matchesToInsert.push({
              id: "match_cornhole_1_1_" + Math.random().toString(36).substr(2, 9),
              stage_id: STAGE5_ID_CONST,
              challenge_id: CORNHOLE_CHALLENGE_ID_CONST,
              round: 1,
              match_index: 1,
            ...
            matchesToInsert.push({
              id: "match_cornhole_1_2_" + Math.random().toString(36).substr(2, 9),
              stage_id: STAGE5_ID_CONST,
              challenge_id: CORNHOLE_CHALLENGE_ID_CONST,
              round: 1,
              match_index: 2,
            ...
            matchesToInsert.push({
              id: "match_cornhole_2_0_" + Math.random().toString(36).substr(2, 9),
              stage_id: STAGE5_ID_CONST,
              challenge_id: CORNHOLE_CHALLENGE_ID_CONST,
              round: 2,
              match_index: 0,
            ...
            matchesToInsert.push({
              id: "match_cornhole_3_0_" + Math.random().toString(36).substr(2, 9),
              stage_id: STAGE5_ID_CONST,
              challenge_id: CORNHOLE_CHALLENGE_ID_CONST,
              round: 3,
              match_index: 0,
            ...
            for (let m = 0; m < 3; m++) {
              matchesToInsert.push({
                id: "match_cornhole_1_" + m + "_" + Math.random().toString(36).substr(2, 9),
                stage_id: STAGE5_ID_CONST,
                challenge_id: CORNHOLE_CHALLENGE_ID_CONST,
                round: 1,
                match_index: m,
        ...
        if (fnName === "submit_cornhole_match_result") {
          ...
          if (match.status === "completed" && match.winner_id === p_winner_id) {
            return { data: db.cornhole_matches.filter((m: any) => m.challenge_id === CORNHOLE_CHALLENGE_ID_CONST), error: null };
          }
          ...
          const challengeMatches = db.cornhole_matches.filter((m: any) => m.challenge_id === CORNHOLE_CHALLENGE_ID_CONST);
          ...
          if (match.round < maxRound) {
            if (match.round === 0) {
              if (match.match_index === 0) {
                const nextMatch = db.cornhole_matches.find(
                  (m: any) => m.challenge_id === CORNHOLE_CHALLENGE_ID_CONST && m.round === 1 && m.match_index === 0
                );
                if (nextMatch) nextMatch.team2_id = p_winner_id;
              } else if (match.match_index === 1) {
                const nextMatch = db.cornhole_matches.find(
                  (m: any) => m.challenge_id === CORNHOLE_CHALLENGE_ID_CONST && m.round === 1 && m.match_index === 1
                );
                if (nextMatch) nextMatch.team1_id = p_winner_id;
              } else if (match.match_index === 2) {
                const nextMatch = db.cornhole_matches.find(
                  (m: any) => m.challenge_id === CORNHOLE_CHALLENGE_ID_CONST && m.round === 1 && m.match_index === 1
                );
                if (nextMatch) nextMatch.team2_id = p_winner_id;
              } else if (match.match_index === 3) {
                const nextMatch = db.cornhole_matches.find(
                  (m: any) => m.challenge_id === CORNHOLE_CHALLENGE_ID_CONST && m.round === 1 && m.match_index === 2
                );
                if (nextMatch) {
                  nextMatch.team1_id = p_winner_id;
                  nextMatch.winner_id = p_winner_id;
                  nextMatch.status = "completed";
                  nextMatch.completed_at = new Date().toISOString();
                }
                const finaleMatch = db.cornhole_matches.find(
                  (m: any) => m.challenge_id === CORNHOLE_CHALLENGE_ID_CONST && m.round === 3 && m.match_index === 0
                );
                if (finaleMatch) finaleMatch.team2_id = p_winner_id;
              }

              for (const nm of db.cornhole_matches.filter((m: any) => m.challenge_id === CORNHOLE_CHALLENGE_ID_CONST && m.round === 1)) {
                if (nm.team1_id && nm.team2_id) nm.status = "ready";
              }
            } else if (match.round === 1) {
              if (match.match_index === 0) {
                const nextMatch = db.cornhole_matches.find(
                  (m: any) => m.challenge_id === CORNHOLE_CHALLENGE_ID_CONST && m.round === 2 && m.match_index === 0
                );
                if (nextMatch) nextMatch.team1_id = p_winner_id;
              } else if (match.match_index === 1) {
                const nextMatch = db.cornhole_matches.find(
              ...
              for (const nm of db.cornhole_matches.filter((m: any) => m.challenge_id === CORNHOLE_CHALLENGE_ID_CONST && m.round === 2)) {
                if (nm.team1_id && nm.team2_id) nm.status = "ready";
              }
              ...
              if (finaleMatch) {
                finaleMatch.team1_id = p_winner_id;
                if (finaleMatch.team1_id && finaleMatch.team2_id) finaleMatch.status = "ready";
              }
            }
          } else {
            // This was the final! Allocate points and complete challenge.
            const pointsAlreadyAssigned = db.scores.some((s: any) => s.challenge_id === CORNHOLE_CHALLENGE_ID_CONST);
            if (!pointsAlreadyAssigned) {
              ...
              const otherTeams = db.teams.filter((t: any) => t.active !== false && t.id !== p_winner_id);
              ...
              activeTeams.forEach((t: any) => {
                let prog = db.team_progress.find((tp: any) => tp.team_id === t.id && tp.challenge_id === CORNHOLE_CHALLENGE_ID_CONST);
                if (!prog) {
                  db.team_progress.push({
          ...
          logActivity(db, p_admin_id, `Match Cornhole ${p_match_id} completato con vincitore ${p_winner_id}`);
          saveDb(db);
          return { data: db.cornhole_matches.filter((m: any) => m.challenge_id === CORNHOLE_CHALLENGE_ID_CONST), error: null };
        }
        ...
        if (fnName === "rollback_cornhole_match_result") {
          ...
          if (match.round < maxRound) {
            ...
            const nextMatch = db.cornhole_matches.find(
              (m: any) => m.challenge_id === CORNHOLE_CHALLENGE_ID_CONST && m.round === nextRound && m.match_index === nextMatchIdx
            );

            if (nextMatch) {
              if (nextMatch.status === "completed") {
                return { data: null, error: { message: "Impossibile annullare: il turno successivo è già stato disputato. Annulla prima quel match." } };
              }
            ...
            const activeTeams = db.teams.filter((t: any) => t.active !== false);
            activeTeams.forEach((t: any) => {
              const prog = db.team_progress.find((tp: any) => tp.team_id === t.id && tp.challenge_id === CORNHOLE_CHALLENGE_ID_CONST);
              if (prog) {
                prog.stato = "started";
        ...
        if (fnName === "get_cornhole_settings") {
          ...
          const tournamentMatches = db.cornhole_matches?.filter((m: any) => m.challenge_id === CORNHOLE_CHALLENGE_ID_CONST) || [];
          const hasStarted = tournamentMatches.some((m: any) => m.status === "completed" && m.team2_id !== null);

          // Find the team that solved Challenge 4.3 first
          const completions = (db.team_progress || [])
            .filter((tp: any) => tp.challenge_id === ENIGMA3_ID && tp.stato === "completed" && tp.completata_at)
            .sort((a: any, b: any) => new Date(a.completata_at).getTime() - new Date(b.completata_at).getTime());
        ...
        if (fnName === "set_cornhole_special_bye") {
          ...
          const hasStarted = tournamentMatches.some((m: any) => m.status === "completed" && m.team2_id !== null);
          if (hasStarted) {
            return { data: null, error: { message: "Impossibile modificare il vantaggio: il torneo è già iniziato." } };
          }

          if (!db.game_settings || db.game_settings.length === 0) {
            db.game_settings = [{ id: "settings_01", marketplace_visible: false, marketplace_active: false, activated_at: null, activated_by: null, cornhole_special_bye_team_id: null }];
          }
          ...
          logActivity(db, p_admin_id, `Regia ha configurato il BYE speciale per la squadra: ${p_team_id || "nessuna"}`);
          saveDb(db);
          return { data: { success: true }, error: null };
        }
        ...
        if (fnName === "generate_boxe_tournament") {
          ...
          const specialByeTeamId = args.p_special_bye_team_id || db.game_settings?.[0]?.boxe_special_bye_team_id || null;
          if (db.game_settings?.[0]) {
          ...
          if (specialByeTeamId && pool.some((t: any) => t.id === specialByeTeamId)) {
            ...
            matchesToInsert.push({
              id: "match_boxe_1_0_" + Math.random().toString(36).substr(2, 9),
              stage_id: STAGE5_ID_CONST,
              challenge_id: BOXE_CHALLENGE_ID_CONST,
              round: 1,
              match_index: 0,
            ...
            matchesToInsert.push({
              id: "match_boxe_1_1_" + Math.random().toString(36).substr(2, 9),
              stage_id: STAGE5_ID_CONST,
              challenge_id: BOXE_CHALLENGE_ID_CONST,
              round: 1,
              match_index: 1,
            ...
            matchesToInsert.push({
              id: "match_boxe_1_2_" + Math.random().toString(36).substr(2, 9),
              stage_id: STAGE5_ID_CONST,
              challenge_id: BOXE_CHALLENGE_ID_CONST,
              round: 1,
              match_index: 2,
            ...
            matchesToInsert.push({
              id: "match_boxe_2_0_" + Math.random().toString(36).substr(2, 9),
              stage_id: STAGE5_ID_CONST,
              challenge_id: BOXE_CHALLENGE_ID_CONST,
              round: 2,
              match_index: 0,
            ...
            matchesToInsert.push({
              id: "match_boxe_3_0_" + Math.random().toString(36).substr(2, 9),
              stage_id: STAGE5_ID_CONST,
              challenge_id: BOXE_CHALLENGE_ID_CONST,
              round: 3,
              match_index: 0,
            ...
            for (let m = 0; m < 3; m++) {
              matchesToInsert.push({
                id: "match_boxe_1_" + m + "_" + Math.random().toString(36).substr(2, 9),
                stage_id: STAGE5_ID_CONST,
                challenge_id: BOXE_CHALLENGE_ID_CONST,
                round: 1,
                match_index: m,
        ...
        if (fnName === "submit_boxe_match_result") {
          ...
          if (match.status === "completed" && match.winner_id === p_winner_id) {
            return { data: db.boxe_matches.filter((m: any) => m.challenge_id === BOXE_CHALLENGE_ID_CONST), error: null };
          }
          ...
          const challengeMatches = db.boxe_matches.filter((m: any) => m.challenge_id === BOXE_CHALLENGE_ID_CONST);
          const maxRound = Math.max(...challengeMatches.map((m: any) => m.round));
          ...
          if (match.round < maxRound) {
            if (match.round === 0) {
              if (match.match_index === 0) {
                const nextMatch = db.boxe_matches.find(
                  (m: any) => m.challenge_id === BOXE_CHALLENGE_ID_CONST && m.round === 1 && m.match_index === 0
                );
                if (nextMatch) nextMatch.team2_id = p_winner_id;
              } else if (match.match_index === 1) {
                const nextMatch = db.boxe_matches.find(
                  (m: any) => m.challenge_id === BOXE_CHALLENGE_ID_CONST && m.round === 1 && m.match_index === 1
                );
                if (nextMatch) nextMatch.team1_id = p_winner_id;
              } else if (match.match_index === 2) {
                const nextMatch = db.boxe_matches.find(
                ...
                const nextMatch = db.boxe_matches.find(
                  (m: any) => m.challenge_id === BOXE_CHALLENGE_ID_CONST && m.round === 1 && m.match_index === 2
                );
                if (nextMatch) {
                  nextMatch.team1_id = p_winner_id;
                  nextMatch.winner_id = p_winner_id;
                ...
                const finaleMatch = db.boxe_matches.find(
                  (m: any) => m.challenge_id === BOXE_CHALLENGE_ID_CONST && m.round === 3 && m.match_index === 0
                );
                if (finaleMatch) finaleMatch.team2_id = p_winner_id;
              }

              for (const nm of db.boxe_matches.filter((m: any) => m.challenge_id === BOXE_CHALLENGE_ID_CONST && m.round === 1)) {
                if (nm.team1_id && nm.team2_id) nm.status = "ready";
              }
            } else if (match.round === 1) {
              if (match.match_index === 0) {
                const nextMatch = db.boxe_matches.find(
                  (m: any) => m.challenge_id === BOXE_CHALLENGE_ID_CONST && m.round === 2 && m.match_index === 0
                );
                if (nextMatch) nextMatch.team1_id = p_winner_id;
              } else if (match.match_index === 1) {
                const nextMatch = db.boxe_matches.find(
              ...
              for (const nm of db.boxe_matches.filter((m: any) => m.challenge_id === BOXE_CHALLENGE_ID_CONST && m.round === 2)) {
                if (nm.team1_id && nm.team2_id) nm.status = "ready";
              }
            } else if (match.round === 2) {
              const finaleMatch = db.boxe_matches.find(
            ...
            // This was the final! Allocate points and complete challenge.
            const pointsAlreadyAssigned = db.scores.some((s: any) => s.challenge_id === BOXE_CHALLENGE_ID_CONST);
            if (!pointsAlreadyAssigned) {
              // 20 points to the winner
              db.scores.push({
              ...
              const activeTeams = db.teams.filter((t: any) => t.active !== false);
              activeTeams.forEach((t: any) => {
                let prog = db.team_progress.find((tp: any) => tp.team_id === t.id && tp.challenge_id === BOXE_CHALLENGE_ID_CONST);
                if (!prog) {
                  db.team_progress.push({
          ...
          logActivity(db, p_admin_id, `Match Boxe Gonfiabile ${p_match_id} completato con vincitore ${p_winner_id}`);
          saveDb(db);
          return { data: db.boxe_matches.filter((m: any) => m.challenge_id === BOXE_CHALLENGE_ID_CONST), error: null };
        }
        ...
        if (fnName === "rollback_boxe_match_result") {
          ...
          if (match.round < maxRound) {
            ...
            const nextMatchIdx = Math.floor(match.match_index / 2);
            const nextMatch = db.boxe_matches.find(
              (m: any) => m.challenge_id === BOXE_CHALLENGE_ID_CONST && m.round === nextRound && m.match_index === nextMatchIdx
            );
            ...
            const activeTeams = db.teams.filter((t: any) => t.active !== false);
            activeTeams.forEach((t: any) => {
              const prog = db.team_progress.find((tp: any) => tp.team_id === t.id && tp.challenge_id === BOXE_CHALLENGE_ID_CONST);
              if (prog) {
                prog.stato = "started";
                prog.completata_at = null;
              }
        ...
        if (fnName === "get_boxe_settings") {
          ...
          const tournamentMatches = db.boxe_matches?.filter((m: any) => m.challenge_id === BOXE_CHALLENGE_ID_CONST) || [];
          const hasStarted = tournamentMatches.some((m: any) => m.status === "completed" && m.team2_id !== null);
        ...
        if (fnName === "set_boxe_special_bye") {
          ...
          if (!db.game_settings || db.game_settings.length === 0) {
            db.game_settings = [{ id: "settings_01", marketplace_visible: false, marketplace_active: false, activated_at: null, activated_by: null, cornhole_special_bye_team_id: null, boxe_special_bye_team_id: null }];
          }
          ...
          logActivity(db, p_admin_id, `Regia ha configurato il BYE speciale Boxe per la squadra: ${p_team_id || "nessuna"}`);
          saveDb(db);
          return { data: { success: true }, error: null };
        }
        ...
        if (fnName === "play_jackpot") {
          ...
          const alreadyPlayed = db.jackpot_plays.some((p: any) => p.team_id === p_team_id && p.challenge_id === JACKPOT_CHALLENGE_ID);
          if (alreadyPlayed) {
          ...
          if (isNaN(puntataNum) || puntataNum < 5 || puntataNum > 20) {
            return { data: null, error: { message: "La puntata deve essere compresa tra 5 e 20 punti." } };
          }

          if (puntataNum > currentScore) {
            return { data: null, error: { message: "Non puoi scommettere più punti di quelli che possiedi." } };
          }
          ...
          const progressIndex = db.team_progress.findIndex((tp: any) => tp.team_id === p_team_id && tp.challenge_id === JACKPOT_CHALLENGE_ID);
          if (progressIndex >= 0) {
          ...
          logActivity(db, p_team_id, `Ha giocato al Jackpot scommettendo ${puntataNum} PT: ${risultato.toUpperCase()} (${sym1}${sym2}${sym3})`);
          saveDb(db);
        ...
        if (fnName === "get_jackpot_state") {
          ...
          const play = db.jackpot_plays.find((p: any) => p.team_id === p_team_id && p.challenge_id === JACKPOT_CHALLENGE_ID) || null;
          const currentScore = db.scores
            ?.filter((s: any) => s.team_id === p_team_id)
            .reduce((sum: number, s: any) => sum + s.punti, 0) || 0;
        ...
        if (fnName === "get_jackpot_plays") {
          ...
          const ADMIN_ID = "11111111-1111-1111-1111-111111111111";
          const isAdminUser = p_admin_id === ADMIN_ID || db.user_roles?.some((ur: any) => ur.user_id === p_admin_id && ur.role === "admin");
          if (!isAdminUser) return { data: null, error: { message: "Non autorizzato", code: "403" } };
          ...
          const plays = db.jackpot_plays.filter((p: any) => p.challenge_id === JACKPOT_CHALLENGE_ID);
        ...
        if (fnName === "get_game_report") {
          ...
          const ADMIN_ID = "11111111-1111-1111-1111-111111111111";
          const isAdmin = p_user_id === ADMIN_ID || db.user_roles?.some((ur: any) => ur.user_id === p_user_id && ur.role === "admin");
          const isPublished = db.game_report?.state === "PUBLISHED_FINAL";
        ...
        if (fnName === "publish_game_report") {
          const { p_admin_id } = args;
          const ADMIN_ID = "11111111-1111-1111-1111-111111111111";
          const isAdmin = p_admin_id === ADMIN_ID || db.user_roles?.some((ur: any) => ur.user_id === p_admin_id && ur.role === "admin");
          if (!isAdmin) {
