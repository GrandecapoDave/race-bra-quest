-- 12_cattiveria_and_tassa_hardening.sql
-- 1) Punti Cattiveria secondo il regolamento (PROJECT_BASELINE.md / CHANGELOG 2026-08-17):
--    - assegnati solo all'utilizzo effettivo, mai per un malus parato dallo Scudo (l'attaccante non guadagna, il difensore prende -3)
--    - Bonus (penalita' cattiveria): bonus_punti -5, ruota_fortuna -2, passaparola -2 (invio richiesta),
--      bonus_classifica -3 (apertura), partenza_anticipata -4 (uso confermato), bonus_scudo -3 (quando para un malus)
--    - Malus (premio cattiveria): freeze_2min +8, trappola +12, penalita_punti +10, tassa_passaggio +15, ruota_sfortunata +7 (al giro della ruota)
--    - cap +30 di punti cattiveria positivi per squadra e tappa (le penalita' negative non sono limitate)
--    - "Chi non e' cattivo paga": alla chiusura tappa (close_stage) 0 malus -> -10, 1 -> 0, 2 -> +5, 3 -> +10, 4 o piu' -> +15
--    - idempotente (riferimento_transazione) e registrato in cattiveria_ledger
--    Prima ogni malus dava un +10 fisso anche se parato e non c'erano penalita' per i bonus ne' regola di fine tappa.
-- 2) Acquisti Marketplace serializzati (advisory lock): con piu' acquisti simultanei (es. 6 Tassa di Passaggio sullo stesso bersaglio)
--    i punti risultavano duplicati/persi. Il bersaglio di un malus deve esistere ed essere una squadra ancora in gara.
-- 3) Ricompensa di tappa con saldo token prima/dopo e nome tappa (il banner "Tappa conclusa" mostrava "Saldo: -> Token" vuoto).

ALTER TABLE public.cattiveria_ledger DROP CONSTRAINT IF EXISTS cattiveria_ledger_tipo_check;
ALTER TABLE public.cattiveria_ledger ADD CONSTRAINT cattiveria_ledger_tipo_check
  CHECK (tipo = ANY (ARRAY['MALUS_UTILIZZATO','SCUDO_ATTIVATO','MALUS_SUBITO_DIFESO','FINE_TAPPA_CATTIVERIA','bonus','malus','end_of_stage']));

CREATE OR REPLACE FUNCTION public.team_current_stage_id(p_team_id uuid)
RETURNS uuid
LANGUAGE sql
STABLE
SECURITY DEFINER
      ...
      WHERE EXISTS (SELECT 1 FROM public.challenges c WHERE c.stage_id = s.id AND c.tipo_sfida <> 'jackpot')
        AND (SELECT COUNT(*) FROM public.challenges c WHERE c.stage_id = s.id AND c.tipo_sfida <> 'jackpot') >
            (SELECT COUNT(DISTINCT tp.challenge_id)
              ...
              WHERE tp.team_id = p_team_id AND c.stage_id = s.id AND c.tipo_sfida <> 'jackpot' AND tp.stato = 'completed')
...
CREATE OR REPLACE FUNCTION public.add_cattiveria(
  p_team_id uuid,
  p_stage_id uuid,
  p_tipo text,
  p_item_id text,
...
AS $function$
DECLARE
  v_positive_sum INTEGER;
  v_allowed INTEGER;
  v_final INTEGER := p_points;
BEGIN
  IF p_team_id IS NULL OR p_points IS NULL OR p_points = 0 THEN
    RETURN;
  END IF;

  PERFORM pg_advisory_xact_lock(hashtext('cattiveria:' || p_team_id::text));

  -- Fine tappa: una sola riga per squadra e tappa
  IF p_tipo = 'end_of_stage' AND EXISTS (
    SELECT 1 FROM public.cattiveria_ledger
    WHERE team_id = p_team_id AND stage_id IS NOT DISTINCT FROM p_stage_id AND tipo = 'end_of_stage'
  ) THEN
    RETURN;
  END IF;

  -- Idempotenza: una transazione genera i suoi punti cattiveria una sola volta per squadra e articolo
  IF p_tx_id IS NOT NULL AND EXISTS (
    SELECT 1 FROM public.cattiveria_ledger
    WHERE team_id = p_team_id
      AND riferimento_transazione = p_tx_id
      AND COALESCE(marketplace_item_id, '') = COALESCE(p_item_id, '')
  ) THEN
    RETURN;
  END IF;

  -- Cap +30 di punti positivi per tappa
  IF p_points > 0 THEN
    SELECT COALESCE(SUM(punti), 0)::INTEGER INTO v_positive_sum
    FROM public.cattiveria_ledger
    WHERE team_id = p_team_id
      AND stage_id IS NOT DISTINCT FROM p_stage_id
      AND punti > 0;
    v_allowed := GREATEST(0, 30 - v_positive_sum);
    v_final := LEAST(p_points, v_allowed);
  END IF;

  INSERT INTO public.cattiveria_ledger (team_id, stage_id, tipo, marketplace_item_id, riferimento_transazione, punti, motivo)
  VALUES (
    p_team_id, p_stage_id, p_tipo, p_item_id, p_tx_id, v_final,
    p_motivo || CASE WHEN v_final <> p_points THEN ' (Cap tappa applicato, punti originali: +' || p_points::text || ')' ELSE '' END
  );
END;
$function$;

REVOKE ALL ON FUNCTION public.add_cattiveria(uuid, uuid, text, text, uuid, integer, text) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.team_current_stage_id(uuid) FROM PUBLIC, anon, authenticated;
...
BEGIN
  ...
  PERFORM public.add_cattiveria(v_team_id, public.team_current_stage_id(v_team_id), 'bonus', 'passaparola', p_transaction_id, -2, 'Utilizzo Passaparola (richiesta inviata)');
...
BEGIN
  ...
  IF v_tx.stato = 'completed' THEN
    ...
    PERFORM public.add_cattiveria(v_team_id, public.team_current_stage_id(v_team_id), 'bonus', 'bonus_classifica', p_transaction_id, -3, 'Utilizzo Bonus Classifica (apertura sessione)');
  END IF;
END;
$function$;
...
BEGIN
  ...
  IF v_tx.marketplace_item_id = 'partenza_anticipata' THEN
    PERFORM public.add_cattiveria(v_tx.team_id, public.team_current_stage_id(v_tx.team_id), 'bonus', 'partenza_anticipata', p_transaction_id, -4, 'Utilizzo Partenza Anticipata (confermata dalla Regia)');
  END IF;
...
BEGIN
  ...
  PERFORM public.add_cattiveria(v_tx.team_id, public.team_current_stage_id(v_tx.team_id), 'malus', 'ruota_sfortunata', v_tx.id, 7, 'Utilizzo Ruota Sfortunata (spin eseguito dal bersaglio)');
...
BEGIN
  ...
  -- Regola "Chi non è cattivo paga" (una sola volta per squadra e tappa)
  FOR v_team IN SELECT * FROM public.teams WHERE active = true LOOP
    DECLARE
      v_malus_count INTEGER;
      v_points INTEGER := 0;
    BEGIN
      SELECT COUNT(*) INTO v_malus_count
      FROM public.cattiveria_ledger
      WHERE team_id = v_team.id AND stage_id = p_stage_id AND tipo = 'malus';

      v_points := CASE
        WHEN v_malus_count = 0 THEN -10
        WHEN v_malus_count = 2 THEN 5
        WHEN v_malus_count = 3 THEN 10
        WHEN v_malus_count >= 4 THEN 15
        ELSE 0
      END;

      IF v_points <> 0 THEN
        PERFORM public.add_cattiveria(
          v_team.id, p_stage_id, 'end_of_stage', NULL,
          NULL,
          v_points,
          'Regola ''Chi non è cattivo paga'': ' || v_malus_count::text || ' Malus usati'
        );
      END IF;
    END;
  END LOOP;
...
BEGIN
  ...
  IF v_total > 0 AND v_done >= v_total AND v_stage_number BETWEEN 1 AND 4 THEN
    ...
    IF NOT EXISTS (
      ...
      UPDATE public.teams SET token_balance = token_balance + v_reward WHERE id = p_team_id RETURNING token_balance INTO v_new_bal;
      ...
      VALUES (
        ...
        jsonb_build_object(
          ...
          'stage_name', v_stage_name,
          'old_balance', v_new_bal - v_reward,
          'new_balance', v_new_bal
        )
      );
