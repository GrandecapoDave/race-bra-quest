-- 16_cattiveria_remaining_malus.sql
-- Punti cattiveria anche per Enigma Extra (+9), Blackout Mercato (+10), Dimezza Punti Tappa (+12):
-- tutti i malus danno punti cattiveria all'attaccante all'applicazione (contano anche in "Chi non e' cattivo paga").
...
BEGIN
  ...
  ELSIF p_item_id = 'enigma_extra' THEN
    PERFORM public.add_cattiveria(v_team_id, public.team_current_stage_id(v_team_id), 'malus', p_item_id, v_tx_id, 9, 'Utilizzo Enigma Extra contro ' || COALESCE(v_target_team_name, ''));
  ELSIF p_item_id = 'blackout_mercato' THEN
    PERFORM public.add_cattiveria(v_team_id, public.team_current_stage_id(v_team_id), 'malus', p_item_id, v_tx_id, 10, 'Utilizzo Blackout Mercato contro ' || COALESCE(v_target_team_name, ''));
  ELSIF p_item_id = 'dimezza_punti' THEN
    PERFORM public.add_cattiveria(v_team_id, public.team_current_stage_id(v_team_id), 'malus', p_item_id, v_tx_id, 12, 'Utilizzo Dimezza Punti Tappa contro ' || COALESCE(v_target_team_name, ''));
  END IF;
