-- Rollback di 06_security_and_stage_arrival.sql: ripristina le definizioni originali (dump di Production del 2026-09-24)
...
BEGIN
  SELECT (public.has_role(auth.uid(), 'admin') OR (p_admin_id IS NOT NULL AND public.has_role(p_admin_id, 'admin')) OR auth.role() = 'service_role') INTO v_is_admin;
...
BEGIN
  ...
  IF v_tx.marketplace_item_id = 'freeze_2min' AND v_tx.target_team_id IS NOT NULL THEN
    UPDATE public.teams
...
DROP FUNCTION IF EXISTS public.assert_admin_caller();
