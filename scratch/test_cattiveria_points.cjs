...
function runTests() {
  ...
  try {
    ...
    teamA.token_balance = 300;
    teamB.token_balance = 300;
    teamC.token_balance = 300;
    ...
    // Team C attacks Team B with freeze -> blocked by shield
    const blockedRes = buyMarketplaceItem(db, teamC.id, "freeze_2min", teamB.id);
    ...
    // Team A attacks Team B with Freeze (shield already consumed)
    const freezeRes = buyMarketplaceItem(db, teamA.id, "freeze_2min", teamB.id);
    if (freezeRes.error) throw new Error(`Freeze error: ${freezeRes.error.message}`);
    ...
    const trappolaRes = buyMarketplaceItem(db, teamA.id, "trappola", teamB.id);
    if (trappolaRes.error) throw new Error(`Trappola error: ${trappolaRes.error.message}`);
    ...
    const penalitaRes = buyMarketplaceItem(db, teamA.id, "penalita_punti", teamB.id);
    if (penalitaRes.error) throw new Error(`Penalità error: ${penalitaRes.error.message}`);
    ...
    const tassaRes = buyMarketplaceItem(db, teamA.id, "tassa_passaggio", teamC.id);
    if (tassaRes.error) throw new Error(`Tassa error: ${tassaRes.error.message}`);
