...
-- 1. Allarga il check constraint su team_social_submissions
ALTER TABLE public.team_social_submissions DROP CONSTRAINT IF EXISTS team_social_submissions_stato_approvazione_check;
ALTER TABLE public.team_social_submissions ADD CONSTRAINT team_social_submissions_stato_approvazione_check CHECK (
  stato_approvazione IS NULL OR stato_approvazione IN ('pending', 'approved', 'rejected', 'confirmed', 'auto_approved')
);

-- 2. Aggiorna evaluate_social_challenge per gestire sia team_social_submissions che submissions
